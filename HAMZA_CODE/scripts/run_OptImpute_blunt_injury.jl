...
data_path = "/pool001/htazi/Trauma/time_split_per_injury_new_morbidity/blunt/"
train_X_time = CSV.read(data_path*"trauma_X_train_time_blunt.csv")
test_X_time = CSV.read(data_path*"trauma_X_test_time_blunt.csv")
...
lnr_optimpute_train = IAI.ImputationLearner(method=:opt_knn, knn_k=k_neighbors_blunt, cluster=true)
...
CSV.write("/pool001/htazi/Trauma/imputed_non_processed/blunt/train_X_time_blunt_imputed.csv",train_X_time)

test_X_time = X_time[n_train+1:n_total, :]
test_X_time = hcat(test_X_time, inc_keys_test)
CSV.write("/pool001/htazi/Trauma/imputed_non_processed/blunt/test_X_time_blunt_imputed.csv",test_X_time)
