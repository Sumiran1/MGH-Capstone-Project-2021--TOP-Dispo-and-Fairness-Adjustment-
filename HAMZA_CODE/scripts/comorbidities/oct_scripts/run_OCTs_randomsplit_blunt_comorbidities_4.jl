...
println("##########################################################################################")
println("##########################################################################################")
println("Starting prediction pipeline for blunt injuries only, morbidity n_4")
println("##########################################################################################")
println("##########################################################################################")
...
train_X_random_morbidity_4 = CSV.read(data_path*"train_X_morbid_4.csv")
test_X_random_morbidity_4 = CSV.read(data_path*"test_X_morbid_4.csv")

# Loading y morbidity 4
train_y_morbidity_random_4 = CSV.read(
    data_path*"train_y_morbid_4.csv",

)
test_y_morbidity_random_4 = CSV.read(
    data_path*"test_y_morbid_4.csv",

)
train_y_morbidity_random_4 = identity.(convert(Matrix, train_y_morbidity_random_4)[:,2])
test_y_morbidity_random_4 = identity.(convert(Matrix, test_y_morbidity_random_4)[:,2])
...
for col_name in severity_categorical_var
    train_X_random_morbidity_4[!, col_name] = CategoricalArray(train_X_random_morbidity_4[!, col_name], ordered=true)
    test_X_random_morbidity_4[!, col_name] = CategoricalArray(test_X_random_morbidity_4[!, col_name], ordered=true)
    println("Transformed column '", col_name, "' to Ordered Categorical")
end
categorical!(train_X_random_morbidity_4)
categorical!(test_X_random_morbidity_4)
...
train_X_random_morbidity_4 = train_X_random_morbidity_4[!, names]
test_X_random_morbidity_4 = test_X_random_morbidity_4[!, names]
...
oct_results_morb = oct(
    train_X_random_morbidity_4,
    test_X_random_morbidity_4,
    train_y_morbidity_random_4,
    test_y_morbidity_random_4,
    seed,
    "hosp_morbidity_4",
    minbucket
)
