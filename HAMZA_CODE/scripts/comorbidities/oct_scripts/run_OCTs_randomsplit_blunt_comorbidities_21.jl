...
println("##########################################################################################")
println("##########################################################################################")
println("Starting prediction pipeline for blunt injuries only, morbidity n_21")
println("##########################################################################################")
println("##########################################################################################")
...
data_path = "/pool001/htazi/Trauma/data_comorbidities_imputed/blunt/"
train_X_random_morbidity_21 = CSV.read(data_path*"train_X_morbid_21.csv")
test_X_random_morbidity_21 = CSV.read(data_path*"test_X_morbid_21.csv")

# Loading y morbidity 4
train_y_morbidity_random_21 = CSV.read(
    data_path*"train_y_morbid_21.csv",

)
test_y_morbidity_random_21 = CSV.read(
    data_path*"test_y_morbid_21.csv",

)
train_y_morbidity_random_21 = identity.(convert(Matrix, train_y_morbidity_random_21)[:,2])
test_y_morbidity_random_21 = identity.(convert(Matrix, test_y_morbidity_random_21)[:,2])
...
for col_name in severity_categorical_var
    train_X_random_morbidity_21[!, col_name] = CategoricalArray(train_X_random_morbidity_21[!, col_name], ordered=true)
    test_X_random_morbidity_21[!, col_name] = CategoricalArray(test_X_random_morbidity_21[!, col_name], ordered=true)
    println("Transformed column '", col_name, "' to Ordered Categorical")
end
categorical!(train_X_random_morbidity_21)
categorical!(test_X_random_morbidity_21)
...
train_X_random_morbidity_21 = train_X_random_morbidity_21[!, names]
test_X_random_morbidity_21 = test_X_random_morbidity_21[!, names]
...
oct_results_morb = oct(
    train_X_random_morbidity_21,
    test_X_random_morbidity_21,
    train_y_morbidity_random_21,
    test_y_morbidity_random_21,
    seed,
    "hosp_morbidity_21",
    minbucket
)
