...
println("##########################################################################################")
println("##########################################################################################")
println("Starting prediction pipeline for blunt injuries only, morbidity n_14")
println("##########################################################################################")
println("##########################################################################################")
...
train_X_random_morbidity_14 = CSV.read(data_path*"train_X_morbid_14.csv")
test_X_random_morbidity_14 = CSV.read(data_path*"test_X_morbid_14.csv")

# Loading y morbidity 4
train_y_morbidity_random_14 = CSV.read(
    data_path*"train_y_morbid_14.csv",

)
test_y_morbidity_random_14 = CSV.read(
    data_path*"test_y_morbid_14.csv",

)
train_y_morbidity_random_14 = identity.(convert(Matrix, train_y_morbidity_random_14)[:,2])
test_y_morbidity_random_14 = identity.(convert(Matrix, test_y_morbidity_random_14)[:,2])
...
for col_name in severity_categorical_var
    train_X_random_morbidity_14[!, col_name] = CategoricalArray(train_X_random_morbidity_14[!, col_name], ordered=true)
    test_X_random_morbidity_14[!, col_name] = CategoricalArray(test_X_random_morbidity_14[!, col_name], ordered=true)
    println("Transformed column '", col_name, "' to Ordered Categorical")
end
categorical!(train_X_random_morbidity_14)
categorical!(test_X_random_morbidity_14)
...
train_X_random_morbidity_14 = train_X_random_morbidity_14[!, names]
test_X_random_morbidity_14 = test_X_random_morbidity_14[!, names]
...
oct_results_morb = oct(
    train_X_random_morbidity_14,
    test_X_random_morbidity_14,
    train_y_morbidity_random_14,
    test_y_morbidity_random_14,
    seed,
    "hosp_morbidity_14",
    minbucket
)
