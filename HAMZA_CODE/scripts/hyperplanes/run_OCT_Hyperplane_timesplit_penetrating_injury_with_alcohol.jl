...
function oct_h(train_X, test_X, train_y, test_y, seed, outcome, minbucket)
    id = "seed=$(seed)___outcome=$(outcome)___minbucket=$(minbucket)___injury=penetrating__hyperplanes"
    outcome = "$(outcome)"
    # Process Data
    Random.seed!(seed)
    @show size(train_X)
...
CSV.write("results_time_split_penetrating_with_alcohol_hyperplanes.csv", df_final_results_time_split)
